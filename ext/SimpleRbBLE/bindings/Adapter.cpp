...
//    rb_gc_mark(self());
        Rice::ruby_mark<SimpleRbBLE::PeripheralRegistry>(_peripheral_registry.get());
//        rb_gc_mark(_peripheral_registry->self());
        if (_on_scan_start) Rice::ruby_mark<SimpleRbBLE::Callback>(_on_scan_start.get());
        if (_on_scan_stop) Rice::ruby_mark<SimpleRbBLE::Callback>(_on_scan_stop.get());
        if (_on_scan_update) Rice::ruby_mark<SimpleRbBLE::Callback>(_on_scan_update.get());
        if (_on_scan_find) Rice::ruby_mark<SimpleRbBLE::Callback>(_on_scan_find.get());
    }
