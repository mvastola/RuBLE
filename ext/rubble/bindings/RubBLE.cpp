...
namespace RubBLE {
    ...
    void Init_BluetoothAddressType() {
        rb_cBluetoothAddressType = Utils::define_described_enum<BluetoothAddressType>("BluetoothAddressType", rb_mRubBLE);
    }
...
//        register_handler<Rice::Exception>(RubBLE::handle_exception<Rice::Exception>);
    }

    void Init_Utils() {
        rb_mUtils = Rice::define_module_under(rb_mRubBLE, "Utils");
    ...
    //static_assert(std::same_as<RubBLE::Identifiers::IDTypes::Full<RubBLE::Adapter, true>::tuple, std::tuple<std::string>>);

    // See https://jasonroelofs.com/rice/4.x
    void Init_RubBLE() {
        RubBLE::main_thread_in_ruby = RubBLE::in_ruby.assert_in_ruby_guard();
        assert(RubBLE::main_thread_in_ruby);
        ...
        rb_mRubBLE = Rice::define_module("RubBLE");
        rubyQueue = RubyQueue::instance();

    #ifdef RUBBLE_DEBUG
        std::cout << "RubyQueue: " << Utils::to_hex_addr(rubyQueue.get()) << std::endl;
        rb_mRubBLE.define_singleton_function("c_debug", &c_debug); // to make it easier to break in gdb from ruby
    #endif
        rb_mRubBLEUnderlying = Rice::define_module_under(rb_mRubBLE, "Underlying");
        Init_Utils();
...
extern "C" [[maybe_unused]] void Init_RubBLE() {
    RubBLE::Init_RubBLE();
}
